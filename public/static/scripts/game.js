...
function updateLoadingStatus (newStatus, progress = 33) {
  ...
  const newValue = Number(progressBar.attr('aria-valuenow')) + progress
...
function hideAll (obj = menus) {
  for (const key in obj) obj[key].hide()
}
function loginSetup () {
...
function updateBar () {
  for (const key in navDetails.resources) navDetails.resources[key].text(' ' + gameData.resources[key])
